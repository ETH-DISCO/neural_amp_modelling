...
class _Layer(_nn.Module):
    def __init__(
        ...
        # NEW: Mixer for the *global* condition vector `g`
        self._global_mixer = Linear(global_condition_size, mid_channels, bias=True)
    ...
    def export_weights(self) -> _torch.Tensor:
        return _torch.cat(
            [
                ...
                self._global_mixer.export_weights(), # Add global mixer weights NEW
    ...
    def forward(
        ...
        zconv = self.conv(x)
        # NEW
        z_local = self._input_mixer(h)[:, :, -zconv.shape[2] :]
        # Transform global condition `g` and prepare for broadcast
        z_global = self._global_mixer(g_global).unsqueeze(-1) # Shape (B, mid_C, 1)

        # Combine all terms
        z_combined = zconv + z_local + z_global
        post_activation = (
    ...
    def import_weights(self, weights: _torch.Tensor, i: int) -> int:
        ...
        i = self._global_mixer.import_weights(weights, i) # Import global mixer weights NEW
