...
class LossConfig(_InitializableFromConfig):
    ...
    normalized_mse_weight: _Optional[float] = None
    ...
    def parse_config(cls, config):
        ...
        return {
            ...
            "normalized_mse_weight": config.get("normalized_mse_weight")
        }
...
class LightningModule(_pl.LightningModule, _InitializableFromConfig):
    ...
    def forward(self, x: _torch.Tensor, g: _torch.Tensor, **kwargs):
        return self.net(x, g, **kwargs)  # TODO deprecate--use self.net() instead.
    ...
    def _shared_step(
        ...
        Always assumes batch is (x, g, y) where x is the input signal, g is the global
        condition, and y is the target signal.
        """
        ...
        x, g, targets = batch
        preds = self(x, g, pad_start=False)
        ...
        normalized_mse_weight = self._loss_config.normalized_mse_weight
        if normalized_mse_weight is not None and normalized_mse_weight > 0.0:
            loss_dict["Normalized MSE"] = _LossItem(
                normalized_mse_weight, self._normalized_mse_loss(preds, targets)
            )
    ...
    def _normalized_mse_loss(
        ...
        :return: ()
        """
        # normalize with rms before computing mse
        N = _torch.sqrt(
            _torch.mean(_torch.square(targets), dim=1, keepdim=True) + 1e-8
        )
        preds = preds / N
        targets = targets / N
        return _nn.MSELoss()(preds, targets)